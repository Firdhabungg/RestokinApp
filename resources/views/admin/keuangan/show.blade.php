            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                    <div class="flex items-start justify-between mb-6">
                        ...
                        <span
                            class="px-3 py-1.5 rounded-full bg-{{ $payment->status_color }}-50 text-{{ $payment->status_color }}-600 text-sm font-bold">
                            ...
                            <div>
                                ...
                                @if ($payment->paid_at)
                                    <p class="text-sm text-gray-600">
                                        ...
                                        <span
                                            class="font-medium text-gray-900">{{ $payment->paid_at->translatedFormat('d F Y, H:i') }}</span>
                                    </p>
                                @endif
                            </div>
                        </div>
                        ...
                        <table class="w-full">
                                ...
                                <tr class="border-t border-gray-100">
                                    <td class="px-4 py-4">
                                        <p class="font-semibold text-gray-900">
                                            {{ $payment->subscription?->plan?->name ?? '-' }}</p>
                                    </td>
                            ...
                            <tfoot class="bg-gray-50">
                                <tr>
                                    ...
                                    <td class="px-4 py-4 text-right font-bold text-xl text-blue-600">
                                        {{ $payment->formatted_amount }}
                                    </td>
                                </tr>
                            </tfoot>
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-lg bg-{{ $payment->status_color }}-50">
                        ...
                        @elseif($payment->status === 'pending')
                            <div
                                class="w-10 h-10 rounded-full bg-{{ $payment->isOverdue() ? 'rose' : 'blue' }}-500 text-white flex items-center justify-center">
                                <i class="fas fa-{{ $payment->isOverdue() ? 'exclamation' : 'clock' }}"></i>
                            </div>
                            <div>
                                <p class="font-bold text-{{ $payment->isOverdue() ? 'rose' : 'blue' }}-700">
                                    {{ $payment->isOverdue() ? 'Jatuh Tempo' : 'Menunggu Pembayaran' }}
                                </p>
                                <p class="text-sm text-{{ $payment->isOverdue() ? 'rose' : 'blue' }}-600">
